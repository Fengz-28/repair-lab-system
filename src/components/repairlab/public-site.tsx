    ...
    <div className={topbarClass}>
      <RepairContainer className="flex min-h-10 flex-wrap items-center justify-between gap-x-6 gap-y-2 py-2 text-xs font-bold sm:text-sm">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
          <span>Atencion por cita</span>
          <span>contacto@fengzlab.tech</span>
          <span className="hidden md:inline">Taller de reparaciÃ³n electrÃ³nica</span>
        </div>
        ...
        <Link className="group flex items-center gap-3" href="/">
          ...
          <span>
            ...
            <span
              ...
              aria-hidden
            >
              <span className="repair-outline-button-label">Taller tÃ©cnico</span>
              <span className="repair-outline-button-hover">Taller tÃ©cnico</span>
            </span>
          </span>
        </Link>
...
export function PublicHero({
  ...
  description,
  primaryHref = "/contact",
  primaryLabel = "Solicitar reparaciÃ³n",
  secondaryHref,
  secondaryLabel,
}: {
  eyebrow: string;
      ...
      <RepairContainer className="relative z-10 grid gap-8 py-12 md:grid-cols-[1.2fr_2fr]">
        <div className="space-y-4">
          ...
          <p className="max-w-md text-sm leading-6 text-zinc-300">
            Taller de reparaciÃ³n electrÃ³nica con diagnÃ³stico, cotizaciÃ³n, seguimiento profesional y entrega documentada.
          </p>
        </div>
        <div className="grid gap-6 sm:grid-cols-3">
          <FooterGroup title="Servicios" items={["DiagnÃ³stico", "Consolas", "Microsoldadura", "Mantenimiento"]} />
          <FooterGroup title="Enlaces rÃ¡pidos" items={["Inicio", "Servicios", "Productos", "Contacto"]} />
          <FooterGroup title="Contacto" items={["contacto@fengzlab.tech", "Atencion por cita", "Lun - Vie"]} />
        </div>
      </RepairContainer>
      <div className="relative z-10 border-t border-white/10 py-5 text-center text-xs text-zinc-500">
        FengzLab. Taller tÃ©cnico con seguimiento transparente.
      </div>
    </footer>
  );
}
...
function TechVisual() {
  const flow = [
    { label: "RecepciÃ³n segura", icon: ClipboardList },
    { label: "DiagnÃ³stico tÃ©cnico", icon: SearchCheck },
    { label: "CotizaciÃ³n transparente", icon: FileText },
      ...
      <div className="repair-premium-card relative rounded-[2rem] border border-white/10 bg-zinc-900/45 p-5 shadow-2xl shadow-black/30 backdrop-blur">
        <div className="rounded-3xl border border-white/10 bg-zinc-950/80 p-5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Mesa tÃ©cnica</span>
